...
class DetailsController extends FrontController
{
	...
	public function index($postId)
	{
		...
		if ($post->pictures->count() > 0) {
			$post->pictures = $post->pictures->take($picturesLimit);
		}
		...
		$description = str_replace('{ad.description}', str(str_strip(strip_tags($post->description)))->limit(200), $description);
		...
		if (empty($description)) {
			$description = str(str_strip(strip_tags($post->description)))->limit(200);
		}
