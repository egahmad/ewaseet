...
class InstallController extends Controller
{
	 ...
	 * @return int
	 */
	public function step(Request $request, $liveData = null): int
	{
		$step = 0;
		...
		$data = $request->session()->get('siteInfo');
		if (isset($data)) {
			$step = 3;
		} else {
			return $step;
		...
		$data = $request->session()->get('databaseImported');
		if (isset($data)) {
			$step = 5;
		} else {
			return $step;
		...
		$data = $request->session()->get('cronJobs');
		if (isset($data)) {
			$step = 6;
		} else {
			return $step;
	...
	public function siteInfo(Request $request)
	{
		...
		$validateDriverParameters = (!empty($mailDriver) && $request->input('validate_driver'));
		...
		if ($request->isMethod('post')) {
			$request->session()->forget('siteInfo');
			...
			$request->session()->put('siteInfo', $siteInfo);
			
			return redirect($this->installUrl . '/database');
		}
		
		$siteInfo = $request->session()->get('siteInfo');
	...
	public function databaseImport(Request $request)
	{
		...
		if ($request->get('action') == 'import') {
			$request->session()->forget('databaseImported');
			
			$this->submitDatabaseImport($request, $siteInfo, $database);
			
			// The database is now imported !
			$request->session()->put('databaseImported', true);
	...
	public function cronJobs(Request $request)
	{
		...
		$request->session()->put('cronJobs', true);
	...
	public function finish(Request $request)
	{
		...
		$request->session()->put('installFinished', true);
