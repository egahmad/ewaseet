...
class InstallationChecker
{
	...
	public function handle(Request $request, Closure $next, $guard = null)
	{
		if ($request->segment(1) == 'install') {
			...
			if (
				!empty($request->session()->get('databaseImported'))
				|| !empty($request->session()->get('cronJobs'))
				|| !empty($request->session()->get('installFinished'))
			) {
	 ...
	 */
	public function alreadyInstalled(Request $request)
	{
		// Check if installation has just finished
		if (
			$request->session()->has('installFinished')
			&& $request->session()->get('installFinished') === true
		) {
			// Write file
			File::put(storage_path('installed'), '');
			
			$request->session()->forget('installFinished');
	 ...
	 * @param Request $request
	 */
	private function checkPurchaseCode(Request $request)
	{
		$tab = [
			'install',
			admin_uri(),
