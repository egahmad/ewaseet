	...
	*/
	...
	public static function selectBoxTree($skippedId = null, $entries = [], &$tab = [], $level = 0, $spacerChars = '-----')
	{
		if (empty($entries)) {
			...
			$entries = self::root()->where('id', '!=', $skippedId)->orderBy('lft')->get();
			if ($entries->count() <= 0) {
				return [];
			}
		}
	 ...
	 * @return array
	 */
	public static function getParentsId($catId, &$parentIds = [])
	{
		$cat = self::find($catId);
		if (!empty($cat)) {
			$parentIds[$cat->id] = $cat->id;
			if (!empty($cat->parent_id)) {
				if (isset($cat->parent) && !empty($cat->parent)) {
					return self::getParentsId($cat->parent->id, $parentIds);
				}
			}
		}
