...
function genPhoneNumberBtn($post, bool $btnBlock = false): string
{
	...
	if (config('settings.single.hide_phone_number')) {
		if (config('settings.single.hide_phone_number') == '1') {
			$phone = str($phone)->mask('X', -str($phone)->length(), str($phone)->length() - 3);
		}
		if (config('settings.single.hide_phone_number') == '2') {
			$phone = str($phone)->mask('X', 3);
		}
		if (config('settings.single.hide_phone_number') == '3') {
			$phone = str($phone)->mask('X', 0);
		}
		$btnLink = '';
 ...
 * @return string
 */
function getPriceLabel(?string $categoryType = null): string
{
 ...
 * @return string
 */
function getPriceInfo($price, ?string $categoryType = null): string
{
	$isNotSalable = ($categoryType == 'not-salable');
	...
	$result = match (true) {
		$isNotSalable => null,
		default => match (true) {
			$isNotFree => \App\Helpers\Number::money($price),
