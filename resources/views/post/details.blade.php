					...
					<nav aria-label="breadcrumb" role="navigation" class="float-start">
						<ol class="breadcrumb">
							...
							@if (isset($catBreadcrumb) && is_array($catBreadcrumb) && count($catBreadcrumb) > 0)
								...
								@endforeach
							@endif
							<li class="breadcrumb-item active" aria-current="page">{{ str($post->title)->limit(70) }}</li>
						</ol>
					</nav>
										...
										<div class="items-details-info col-md-12 col-sm-12 col-12 enable-long-words from-wysiwyg">
											...
											<div class="row">
												...
												<div class="col-md-6 col-sm-6 col-6 text-end">
													<h4 class="fw-normal p-0">
														<span class="fw-bold">
															{{ getPriceLabel($post->category->type) }}
														</span>
														<span>
															{!! getPriceInfo($post->price, $post->category->type) !!}
				...
				<div class="col-lg-3 page-sidebar-right">
					<aside>
						...
						@if (config('settings.single.show_listing_on_googlemap'))
							<?php
							$mapHeight = 250;
							$mapPlace = (isset($post->city) && !empty($post->city))
								? $post->city->name . ',' . config('country.name')
								: config('country.name');
